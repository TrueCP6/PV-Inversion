...
\begin{equation}\label{p_bar}
    \overline{p}(z) = \overline {p}_\text{bot} \left( 1- \frac{\kappa g}{R} \left( \frac{p_\text{surf}}{\overline{p}(0)}\right)^\kappa \int_0^z \frac{d \xi}{\overline{\theta}(\xi)}\right)^{1/\kappa}
\end{equation}
...
To calculate the source term $q(x,y,z)$ we must first specify the background state $\overline{Q}$. This is given by \cite{fehlmann1997pvelements}:

\begin{equation}\label{Q_bar} %TODO check relative vorticity is not necessary here
    \overline{Q}(z) = \frac{f \overline{N}^2\overline{\theta}}{\overline{\rho} g}
\end{equation}
